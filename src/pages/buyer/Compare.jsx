...
import { X, BadgeCheck, Star, AlertTriangle, Home } from 'lucide-react';
              ...
              {properties.map((p) => {
                ...
                return (
                  <th key={p.id} className="min-w-[200px] border-b border-gray-100 bg-gray-50 p-3 text-left align-top">
                    ...
                    {primaryImage ? (
                      <img src={resolveMediaUrl(primaryImage.url)} alt={title} className="mt-2 h-28 w-full rounded-lg object-cover" />
                    ) : (
                      <div className="mt-2 flex h-28 w-full items-center justify-center rounded-lg bg-brand-50 text-brand-300">
                        <Home size={32} className="stroke-[1.2]" />
                      </div>
                    )}
