...
const ERROR_KEYS = {
  INVALID_CREDENTIALS: 'portal.invalidManagerCredentials',
  ACCOUNT_PENDING: 'portal.managerAccountInactive',
  ACCOUNT_REJECTED: 'portal.managerAccountInactive',
  ACCOUNT_INACTIVE: 'portal.managerAccountInactive',
...
export default function ManagerLogin() {
  ...
  async function handleSubmit({ id, password }) {
    ...
    setSubmitting(true);
    try {
      const user = await authService.loginManager(id, password, rememberMe);
      setUser(user);
      ...
      setError(
        t(
          ERROR_KEYS[err.code] ||
            'portal.invalidManagerCredentials'
        )
      );
    } finally {
      setSubmitting(false);
  ...
  async function handleResetSubmit(e) {
    ...
    if (!resetId.trim()) {
      setResetError('Please enter your Manager ID or Mobile Number.');
      return;
    }
    ...
    try {
      await authService.resetManagerPassword(resetId.trim(), newPassword);
      setResetSuccess(true);
      toast.success('Manager password updated successfully!');
      setTimeout(() => {
        setManagerId(resetId.trim());
        setShowForgotModal(false);
      }, 1500);
    } catch (err) {
      setResetError(err.message || 'Failed to update password. Please verify your Manager ID.');
    } finally {
  ...
  return (
    <>
      <PortalLoginCard
        heading={t('portal.managerPortalHeading')}
        secureMessage={t('portal.secureManagerAccess')}
        idLabel={t('portal.managerId')}
        idValue={managerId}
        onIdChange={setManagerId}
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/50 p-4">
          <div className="relative w-full max-w-md rounded-2xl bg-warm-white p-6 shadow-2xl sm:p-8">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold text-brand-900">Reset Manager Password</h2>
              ...
              <form onSubmit={handleResetSubmit} className="mt-6 space-y-4">
                <div>
                  <label className="mb-1 block text-xs font-semibold text-gray-700">
                    Manager ID / Mobile Number
                  </label>
                  <input
                    type="text"
                    required
                    placeholder="e.g. MGR-2026-000001 or 9000000002"
                    value={resetId}
