...
const ERROR_KEYS = {
  ...
  ACCOUNT_PENDING: 'portal.adminAccountInactive',
  ACCOUNT_REJECTED: 'portal.adminAccountInactive',
...
export default function AdminLogin() {
  ...
  async function handleSubmit({ id, password }) {
    ...
    try {
      ...
      setError(
        t(
          ERROR_KEYS[err.code] ||
            'portal.invalidAdminCredentials'
        )
      );
    } finally {
      setSubmitting(false);
  ...
  async function handleResetSubmit(e) {
    ...
    if (!resetId.trim()) {
      setResetError('Please enter your Admin Login ID or Mobile Number.');
      return;
    }
    ...
    try {
      await authService.resetAdminPassword(resetId.trim(), newPassword);
      setResetSuccess(true);
      toast.success('Admin password updated successfully!');
      setTimeout(() => {
        setLoginId(resetId.trim());
        setShowForgotModal(false);
      }, 1500);
    } catch (err) {
      setResetError(err.message || 'Failed to update admin password. Please verify your Login ID.');
    } finally {
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/50 p-4">
          <div className="relative w-full max-w-md rounded-2xl bg-warm-white p-6 shadow-2xl sm:p-8">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold text-brand-900">Reset Admin Password</h2>
            ...
            {resetSuccess ? (
              <div className="mt-6 text-center">
                ...
                <p className="mt-1 text-xs text-gray-500">
                  Your admin password has been successfully updated. You can now login with your new credentials.
                </p>
              </div>
            ) : (
              <form onSubmit={handleResetSubmit} className="mt-6 space-y-4">
                <div>
                  <label className="mb-1 block text-xs font-semibold text-gray-700">
                    Admin Login ID / Mobile Number
                  </label>
                  <input
                    type="text"
                    required
                    placeholder="e.g. ADMIN001 or 9000000001"
                    value={resetId}
