...
export default function Footer() {
  ...
  const phone = cms?.contactPhone || '+91 90000 00000';
  const email = cms?.contactEmail || 'contact@omkareswarrealtors.demo';
  const address = language === 'te'
    ? (cms?.contactAddressTe || cms?.contactAddressEn || 'బ్రోడిపేట, గుంటూరు, ఆంధ్రప్రదేశ్ - 522002')
    : (cms?.contactAddressEn || 'Brodipet, Guntur, Andhra Pradesh - 522002');
  const mapUrl = cms?.contactMapUrl || `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(address)}`;

  const socialLinks = [
    { icon: FacebookIcon, label: 'Facebook', href: cms?.socialFacebook || '#' },
    { icon: InstagramIcon, label: 'Instagram', href: cms?.socialInstagram || '#' },
    { icon: TwitterXIcon, label: 'Twitter', href: cms?.socialTwitter || '#' },
    { icon: YoutubeIcon, label: 'YouTube', href: cms?.socialYoutube || '#' },
  ];
          ...
          <ul className="mt-2.5 space-y-2 text-sm text-brand-200">
            <li className="flex items-start gap-2">
              ...
              <a
                ...
                className="hover:text-warm-white hover:underline transition-colors lang-te"
              >
                {address}
              </a>
            </li>
            <li className="flex items-center gap-2">
              ...
              <a
                ...
                className="lang-te block hover:text-warm-white transition-colors"
              >
                {phone}
              </a>
            </li>
            <li className="flex items-center gap-2">
              ...
              <a
                href={`mailto:${email}`}
                className="lang-te block hover:text-warm-white transition-colors break-all"
              >
                {email}
              </a>
            </li>
            ...
            {socialLinks.map(({ icon: Icon, label, href }) => {
              ...
              return (
                <a
                  key={label}
                  href={href}
                  target={isDummy ? undefined : '_blank'}
                  rel={isDummy ? undefined : 'noopener noreferrer'}
                  aria-label={label}
