...
export default function CallNoteTimeline({ enquiryId }) {
  ...
  function load() {
    callNoteService.getForEnquiry(enquiryId).then(setNotes);
  }
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="mt-3 space-y-2 rounded-xl border border-dashed border-brand-300 p-3">
          ...
          <input
            ...
            className="w-full rounded-lg border border-gray-300 px-2 py-1.5 text-xs"
          />
          <button type="submit" className="rounded-lg bg-brand-700 px-3 py-1.5 text-xs font-semibold text-warm-white">
            {t('callNote.save')}
          </button>
        </form>
      )}
