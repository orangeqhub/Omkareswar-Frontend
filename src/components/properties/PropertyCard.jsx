...
export default function PropertyCard({ property }) {
  ...
  return (
    <div className="group relative flex flex-col overflow-hidden rounded-xl border border-gray-200 bg-warm-white shadow-sm transition-shadow hover:shadow-md h-full">
      <Link to={`/properties/${property.id}`} className="relative block h-44 w-full overflow-hidden bg-gray-100 shrink-0">
        {primaryImage ? (
          <img
          ...
          <div className="flex h-full w-full items-center justify-center bg-brand-50 text-brand-300">
            <Home size={44} className="stroke-[1.2]" />
          </div>
        )}
      ...
      <div className="flex flex-1 flex-col p-4 justify-between">
        <div className="flex flex-col gap-1">
          ...
          <p className="flex items-center gap-1 text-sm text-gray-500 min-h-[20px]">
            <MapPin size={14} className="shrink-0" /> <span className="line-clamp-1 lang-te">{location || '—'}</span>
          </p>
          {property.distanceKm != null ? (
            <p className="lang-te text-xs font-medium text-brand-700 min-h-[16px]">
          ...
          <p className="flex items-center gap-1 text-sm text-gray-500">
            <Ruler size={14} className="shrink-0" /> {property.area} {property.areaUnit}
          </p>
